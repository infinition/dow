// @ts-check
import { signal, useSignal } from '../html.js';
...
// Bumped (coalesced to once per frame) whenever new image stats are cached, so
// the size/dimension sorting in data.js can re-run as measurements arrive.
export const statsCacheVersion = signal(0);

/** @returns {ImageStatsData | undefined} */
export function getCachedStats(/** @type {string} */ url) {
	return cache.get(url);
}

let bumpScheduled = false;
function scheduleStatsBump() {
	if (bumpScheduled) return;
	bumpScheduled = true;
	requestAnimationFrame(() => {
		bumpScheduled = false;
		statsCacheVersion.value++;
	});
}
...
export function useImageStats(/** @type {string} */ imageUrl) {
	...
	function onLoad(/** @type {Event} */ e) {
		...
		// Also key by the original URL passed to the hook so sorting can look it
		// up by the same string that lives in the image lists.
		if (imageUrl && imageUrl !== url) cache.set(imageUrl, loaded);
		data.value = loaded;
		scheduleStatsBump();
