...
import { getCachedStats, statsCacheVersion } from './useImageStats.js';
...
// Images
/** @type {import('/lib/preact-signals.mjs').Signal<string[]>} */
...
// Sorting: cycles none → size (KB) → width → height. All descending (biggest
// first). Dimensions come from the loaded <img> elements; file size from the
// stats cache, re-sorting as measurements arrive (statsCacheVersion).
/** @type {import('/lib/preact-signals.mjs').Signal<'none' | 'size' | 'width' | 'height'>} */
export const sortMode = signal('none');

export const sortedDisplayedImages = computed(() => {
	const images = displayedImages.value;
	const mode = sortMode.value;
	if (mode === 'none') return images;

	statsCacheVersion.value; // subscribe: re-sort when new stats are measured
	const cacheElement = imagesCache.value;

	const measure = (/** @type {string} */ url) => {
		const stats = getCachedStats(url);
		if (mode === 'size') return stats?.size?.bytes || 0;

		if (stats && (mode === 'width' ? stats.width : stats.height)) {
			return mode === 'width' ? stats.width : stats.height;
		}
		const image = /** @type {HTMLImageElement | null} */ (
			cacheElement?.querySelector(`img[src="${url}"]`)
		);
		return mode === 'width' ? image?.naturalWidth || 0 : image?.naturalHeight || 0;
	};

	return [...images].sort((a, b) => measure(b) - measure(a));
});
